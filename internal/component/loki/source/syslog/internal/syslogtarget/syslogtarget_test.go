...
func TestSyslogTarget_HostnameFallbackToIP(t *testing.T) {
	...
	// "sshd[78988]:" matches the process[pid] bad hostname pattern
	messages := []string{
		`<165>1 2018-10-11T22:14:15.003Z sshd[78988]: app - id1 [custom@32473 exkey="1"] Message with bad hostname`,
		`<165>1 2018-10-11T22:14:15.005Z validhost app - id2 [custom@32473 exkey="2"] Message with valid hostname`,
	}
	...
	// Check that bad hostname was replaced with connection IP (127.0.0.1)
	// and valid hostname was preserved
	received := handler.Received()
	require.Len(t, received, 2)

	// First message has bad hostname pattern "sshd[78988]:" - should be replaced with IP
	require.Equal(t, "127.0.0.1", string(received[0].Labels["hostname"]), "Bad hostname should be replaced with connection IP")

	// Second message has valid hostname "validhost" - should be preserved
	require.Equal(t, "validhost", string(received[1].Labels["hostname"]), "Valid hostname should be preserved")
}
...
func TestSyslogTarget_HostnameFallbackDisabled(t *testing.T) {
	...
	hostname := string(handler.Received()[0].Labels["hostname"])
...
func TestSyslogTarget_HostnameFallbackMissingHostname(t *testing.T) {
	...
	messages := []string{
		`<165>1 2018-10-11T22:14:15.003Z - app - id1 [custom@32473 exkey="1"] Message without hostname`,
	}
